...
include("../config/db.php");
$rows  = mysqli_query($conn, "SELECT * FROM staff WHERE role='pharmacist' ORDER BY created_at DESC");
$total = mysqli_fetch_assoc(mysqli_query($conn,"SELECT COUNT(*) as t FROM staff WHERE role='pharmacist'"))['t'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>Pharmacists | CareFlow Admin</title>
...
.stat-pill { background: #fef3c7; color: #b45309; font-size: 13px; font-weight: 600; padding: 5px 14px; border-radius: 20px; }
...
.staff-avatar { width: 44px; height: 44px; border-radius: 50%; background: #f59e0b; color: #fff; font-size: 15px; font-weight: 700; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
...
<div class="sidebar">
  ...
  <a href="pharmacist.php" class="active">💊 Pharmacy</a>
...
<div class="main">
  <div class="page-header">
    ...
    <a href="add_staff.php" class="add-btn">➕ Add Pharmacist</a>
  </div>
  <div class="table-card">
    <?php if ($total == 0): ?>
    <div class="empty-state"><div>💊</div>No pharmacists added yet.</div>
    <?php else: ?>
